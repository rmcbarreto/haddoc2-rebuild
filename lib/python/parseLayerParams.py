...
def write_bias_value(data, name, nbits, target):
    ...
    print("bias_fp = {}".format(bias_fp))

    target.write(" (")
    for n in range(out_size):
        # As dimensões do kernel_fp nunca pode ultrapassar o valor do scale_factor que para 4 bits e 7, ou seja so usa 3 bits
        if (bias_fp[n] > scale_factor):
            bias_fp[n] = scale_factor
        if (bias_fp[n] < -scale_factor):
            bias_fp[n] = -scale_factor
        bias_bin = np.binary_repr(bias_fp[n], width=nbits)
        print("bias_bin[{}] = {}".format(n,bias_bin))
...
def write_kernel_value(kernel_data, layer_name, nbits, target):
    ...
    scale_factor = to_scaleFactor(nbits) # se nbits = 4 -> scale_facor = 2^{4-1}-1 = 7
    ...
    kernel_fp = to_fixedPoint(kernel_data, scale_factor) # return(round(scale_factor*kernel_data)) em formato inteiro.
    target.write(" (")
        ...
        for m in range(in_size):
            for i in range(kernel_size - 1, -1, -1):
                for j in range(kernel_size - 1, -1, -1):
                    ...
                    if (kernel_fp[n][m][i][j] < -scale_factor):
                        kernel_fp[n][m][i][j] = -scale_factor
                    kernel_bin = np.binary_repr(kernel_fp[n][m][i][j], width=nbits) # vai passar a representacao de decimal para binario, usando o bit extra como bit de sinal
